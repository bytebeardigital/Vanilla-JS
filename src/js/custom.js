...
function getFinances(data) {
  ...
  //lets get the FinanceData
  let financeData = dashboardData
    .filter((info) => info.type == "finances")
    .map((data) => {
  ...
  let expenseData = expense.expense;

  //p element on the board
  var incomeEl = document.getElementById("incomeTotal");
  var expenseEl = document.getElementById("expenseTotal");

  // totalIncome element set to 0 instead of null;
  let totalIncome = 0;
  let totalExpenses = 0;

  // loop through incomeData and add the income_price elements together;
